...
// Modify the function cf7d_get_db_fields to return the desired fields
if ( ! function_exists('cf7d_get_db_fields') ) {
	function cf7d_get_db_fields( $fid, $filter = true ) {
		global $wpdb;
		$sql  = sprintf('SELECT `name` FROM `' . $wpdb->prefix . 'cf7_data_entry_%d` WHERE `name` IN ("firstname", "lastname", "service", "quantity", "tva", "price_total", "sumtion_data") GROUP BY `name`', $fid);
		$data = $wpdb->get_results($sql);
...
// Modify the function cf7d_sortdata to return the desired fields
if ( ! function_exists('cf7d_sortdata') ) {
	function cf7d_sortdata( $data ) {
		$data_sorted = array();
		foreach ( $data as $k => $v ) {
			...
			if (in_array($v->name, array("firstname", "lastname", "service", "quantity", "tva", "price_total", "sumtion_data"))) {
				$data_sorted[ $v->data_id ][ $v->name ] = apply_filters('cf7d_entry_value', $v->value, $v->name);
			}
		}
