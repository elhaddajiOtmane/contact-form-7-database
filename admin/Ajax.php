			...
			if ( isset( $_POST['field'] ) &&
				...
				if ( $checkUpOption || $checkAddOption ) {
					...
					wp_send_json_success(
						...
						add_action( 'wp_ajax_cf7d_export_to_csv', 'cf7d_export_to_csv' );
						if ( ! function_exists( 'cf7d_export_to_csv' ) ) {
						function cf7d_export_to_csv() {
						try {
							if (
									isset( $_POST['fid'] ) &&
									isset( $_POST['ids_export'] )
							) {
									// check nonce
									cf7d_checkNonce();
								...
								if ( isset( $_POST['search'] ) ) {
									$search = sanitize_text_field( addslashes( $_POST['search'] ) );
								}
								if ( isset( $_POST['from_date'] ) && isset( $_POST['to_date'] ) ) {
									...
									$search_to_date   = new DateTime( $search_to_date );
									$search_to_date   = $search_to_date->format( 'Y-m-d' );
									$query            = sprintf( 'SELECT * FROM `' . $wpdb->prefix . 'cf7_data_entry` WHERE `cf7_id` = %d AND data_id IN(SELECT * FROM (SELECT data_id FROM `' . $wpdb->prefix . 'cf7_data_entry` WHERE 1 = 1 AND `cf7_id` = ' . $fid . "  ". ( ( ! empty( $search ) ) ? "AND `value` LIKE '%%" . $search . "%%'" : '' ) . " AND name='submit_time' AND CONVERT(value , DATE) >= " . "'" . $search_form_date . "'" . ' AND CONVERT(value, DATE) <= ' . "'" . $search_to_date . "'" . ' GROUP BY `data_id` ORDER BY ' . $cf7d_entry_order_by . ') temp_table) ORDER BY ' . $cf7d_entry_order_by, $fid );
									$data             = $wpdb->get_results( $query );
									$data_sorted      = apply_filters( 'cf7d_ad_before_printing_data', cf7d_sortdata( $data ), $fid );
								} else {
									$query       = sprintf( 'SELECT * FROM `' . $wpdb->prefix . 'cf7_data_entry` WHERE `cf7_id` = %d AND data_id IN(SELECT * FROM (SELECT data_id FROM `' . $wpdb->prefix . 'cf7_data_entry` WHERE 1 = 1 AND `cf7_id` = ' . $fid . ' ' . ( ( ! empty( $search ) ) ? "AND `value` LIKE '%%" . $search . "%%'" : '' ) . ' GROUP BY `data_id` ORDER BY ' . $cf7d_entry_order_by . ') temp_table) ORDER BY ' . $cf7d_entry_order_by, $fid );
									$data        = $wpdb->get_results( $query );
									$data_sorted = apply_filters( 'cf7d_ad_before_printing_data', cf7d_sortdata( $data ), $fid );
									// $ids_export = cf7d_sanitize_arr( json_decode( stripslashes( $_POST['ids_export'] ), true ) );
									// $ids_export = implode( ',', array_map( 'intval', $ids_export ) );
									// $data        = cf7d_get_entrys( $fid, $ids_export, 'data_id desc' );
									// $data_sorted = apply_filters( 'cf7d_ad_before_printing_data', cf7d_sortdata( $data ), $fid );
								}

									// get file name
									$file_name = 'cf7-database';
									$forms     = get_posts(
										array(
											'post_status'    => 'any',
											'posts_per_page' => -1,
								...
								foreach ( $forms as $k => $form ) {
									...
									$conFieldsArr   = array();
									$name_fields    = array('firstname', 'lastname', 'service', 'quantity', 'phonenumber', 'tva', 'price_total', 'sumtion_data');
									$conFieldsArr[] = $name_fields;

								foreach ( $data_sorted as $k => $v ) {
									$temp_value = array();
									foreach ( $fields as $k2 => $v2 ) {
										if ($k2 == 'firstname' || $k2 == 'lastname' || $k2 == 'service' || $k2 == 'quantity'  || $k2 == 'phonenumber' || $k2 == 'tva' || $k2 == 'price_total' || $k2 == 'sumtion_data') {
											$temp_value[] = ( ( isset( $v[ $k2 ] ) ) ? stripcslashes( $v[ $k2 ] ) : '' );
										}
									}
									$conFieldsArr[] = $temp_value;
								}

									wp_send_json_success(
										array(
											'mess' => __( 'Export successfully.', 'cf7-database' ),
											array(
												'result'   => $conFieldsArr,
												'fileName' => $file_name,
											),
